...
func CreateUser() gin.HandlerFunc {
	return func(c *gin.Context) {
		...
		if err := c.BindJSON(&user); err != nil {
			c.JSON(http.StatusBadRequest, responses.UserResponse{Status: http.StatusBadRequest, Message: err.Error(), Data: nil})
			return
		}

		//use the validator library to validate required fields
		if validationErr := validate.Struct(&user); validationErr != nil {
			c.JSON(http.StatusBadRequest, responses.UserResponse{Status: http.StatusBadRequest, Message: validationErr.Error(), Data: nil})
			return
		}
...
func EditAUser() gin.HandlerFunc {
	return func(c *gin.Context) {
		...
		newUser := bson.M{"name": user.Name, "avatarUrl": user.AvatarUrl}
		result, err := userCollection.UpdateOne(ctx, bson.M{"id": objId}, bson.M{"$set": newUser})
